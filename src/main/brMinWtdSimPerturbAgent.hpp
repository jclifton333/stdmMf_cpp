#ifndef BR_MIN_WTD_SIM_PERTURB_AGENT_HPP
#define BR_MIN_WTD_SIM_PERTURB_AGENT_HPP
...
class BrMinWtdSimPerturbAgent : public Agent<State> {
    ...
    const uint32_t num_sim_eps_;
    const uint32_t num_sim_obs_;
...
public:
    BrMinWtdSimPerturbAgent(const std::shared_ptr<const Network> & network,
    ...
    BrMinWtdSimPerturbAgent(const BrMinWtdSimPerturbAgent<State> & other);

    ~BrMinWtdSimPerturbAgent() override = default;
...
#endif // BR_MIN_WTD_SIM_PERTURB_AGENT_HPP
