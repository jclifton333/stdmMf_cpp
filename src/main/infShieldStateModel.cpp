...
InfShieldStateModel::InfShieldStateModel(const InfShieldStateModel & other)
    : Model<InfShieldState>(other.par_size_, other.network_) {
}
...
std::vector<double> InfShieldStateModel::probs(
    ...
    for (uint32_t i = 0; i < this->num_nodes_; ++i) {
        ...
        if (status_i < 2) {
            ...
            double prob = 1.0 - this->inf_b(i, trt_i, state,
            ...
            const double prob = this->rec_b(i, trt_i, state, trt_status);
...
double InfShieldStateModel::ll(
        const std::vector<Transition<InfShieldState> > & history) const {
    const uint32_t history_size = history.size();
    CHECK_GE(history_size, 1);
    double ll_value = 0.0;
    for (uint32_t i = 0; i < history_size; ++i) {
        // split up transition
        const Transition<InfShieldState> & transition = history.at(i);
        const InfShieldState & curr_state = transition.curr_state;
        const boost::dynamic_bitset<> & curr_trt =
            transition.curr_trt_bits;
        const InfShieldState & next_state = transition.next_state;
        ...
        // likelihood for infection transition
        ...
        // likelihood for shield transition
        for (uint32_t j = 0; j < this->num_nodes_; ++j) {
            ll_value += this->shield_prob(j, curr_state, next_state, true);
        }
    }
...
std::vector<double> InfShieldStateModel::ll_grad(
        const std::vector<Transition<InfShieldState> > & history) const {
    const uint32_t history_size = history.size();
    ...
    for (uint32_t i = 0; i < history_size; ++i) {
        ...
        for (uint32_t j = 0; j < this->num_nodes_; ++j) {
            ...
            // gradient for shield transition
            const std::vector<double> shield_grad_val = this->shield_grad(j,
                    curr_state, next_state);

            njm::linalg::add_b_to_a(grad_value, shield_grad_val);
        }
    }
...
std::vector<double> InfShieldStateModel::ll_hess(
        const std::vector<Transition<InfShieldState> > & history) const {
    const uint32_t history_size = history.size();
    CHECK_GT(history.size(), 0);
        ...
        for (uint32_t j = 0; j < this->num_nodes_; ++j) {
            ...
            if (status_j < 2) {
                ...
                if (prob_j > 0.0 && prob_j < 1.0) {
                    // latent prob
                    const double prob_j0 = this->inf_b(j, trt_j, curr_state,
                            curr_trt);
            ...
            // hessian for shield transition
            const std::vector<double> shield_hess_val = this->shield_hess(j,
                    curr_state, next_state);
            njm::linalg::add_b_to_a(hess_val, shield_hess_val);
        }
    }
...
InfShieldState InfShieldStateModel::turn_clock(
    ...
    for (uint32_t i = 0; i < this->num_nodes_; ++i) {
        ...
        next_state.shield.at(i) = this->shield_draw(i, curr_state);
    }
