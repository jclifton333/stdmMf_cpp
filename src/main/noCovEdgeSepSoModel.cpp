...
std::shared_ptr<Model> NoCovEdgeSepSoModel::clone() const {
    return std::shared_ptr<Model>(new NoCovEdgeSepSoModel(*this));
}


std::vector<double> NoCovEdgeSepSoModel::par() const {
...
void NoCovEdgeSepSoModel::par(const std::vector<double> & par) {
...
uint32_t NoCovEdgeSepSoModel::par_size() const {
    return this->par_size_;
}


std::vector<double> NoCovEdgeSepSoModel::probs(
        ...
        for (uint32_t j = 0; j < this->num_nodes_; ++j) {
            ...
            if (status_j < 2) {
                ...
                if (prob_j > 0.0 && prob_j < 1.0) {
                    ...
                    std::vector<double> outer_grad_j(njm::linalg::outer_a_and_b(
                                    grad_j, grad_j));
...
double NoCovEdgeSepSoModel::inf_b(const uint32_t & b_node,
        const bool & b_trt,
...
double NoCovEdgeSepSoModel::a_inf_b(const uint32_t & a_node,
...
double NoCovEdgeSepSoModel::rec_b(const uint32_t & b_node,
        const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
...
std::vector<double> NoCovEdgeSepSoModel::inf_b_grad(const uint32_t & b_node,
        const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
...
std::vector<double> NoCovEdgeSepSoModel::a_inf_b_grad(
...
std::vector<double> NoCovEdgeSepSoModel::rec_b_grad(
...
std::vector<double> NoCovEdgeSepSoModel::inf_b_hess(const uint32_t & b_node,
        const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
...
std::vector<double> NoCovEdgeSepSoModel::a_inf_b_hess(
        const uint32_t & a_node, const uint32_t & b_node,
        const bool & a_trt, const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
...
std::vector<double> NoCovEdgeSepSoModel::rec_b_hess(
        const uint32_t & b_node, const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
