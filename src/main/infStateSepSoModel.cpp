...
InfStateSepSoModel::InfStateSepSoModel(
        const std::shared_ptr<const Network> & network)
    : InfStateModel(9, network), intcp_inf_latent_(0.0), intcp_inf_(0.0),
      intcp_rec_(0.0), trt_act_inf_(0.0), trt_act_inf_so_(0.0),
      trt_act_rec_(0.0), trt_act_rec_so_(0.0), trt_pre_inf_(0.0),
      trt_pre_inf_so_(0.0) {
}

InfStateSepSoModel::InfStateSepSoModel(const InfStateSepSoModel & other)
    : InfStateModel(other), intcp_inf_latent_(other.intcp_inf_latent_),
...
std::shared_ptr<Model<InfState> > InfStateSepSoModel::clone() const {
    return std::shared_ptr<Model<InfState> >(new InfStateSepSoModel(*this));
}


std::vector<double> InfStateSepSoModel::par() const {
    ...
    par.push_back(this->trt_act_inf_so_);
...
void InfStateSepSoModel::par(const std::vector<double> & par) {
...
double InfStateSepSoModel::inf_b(const uint32_t & b_node,
        const bool & b_trt,
...
double InfStateSepSoModel::a_inf_b(const uint32_t & a_node,
    ...
    const double base = this->intcp_inf_ + this->trt_act_inf_ * a_trt
        + this->trt_act_inf_so_ * a_trt_so + this->trt_pre_inf_ * b_trt
        + this->trt_pre_inf_so_ * b_trt_so;
    LOG_IF(FATAL, !std::isfinite(base)) << "base is not finite.";
...
double InfStateSepSoModel::rec_b(const uint32_t & b_node,
        const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
    ...
    const double base = this->intcp_rec_ + this->trt_act_rec_ * b_trt
        + this->trt_act_rec_so_ * b_trt_so;
    LOG_IF(FATAL, !std::isfinite(base)) << "base is not finite.";

    return 1.0 - 1.0 / (1.0 + std::exp(std::min(100.0, base)));
}

std::vector<double> InfStateSepSoModel::inf_b_grad(const uint32_t & b_node,
        const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
    ...
    grad_val.at(0) = val;
    grad_val.at(7) = b_trt * val;
    grad_val.at(8) = b_trt_so * val;
    return grad_val;
}

std::vector<double> InfStateSepSoModel::a_inf_b_grad(
    ...
    grad_val.at(4) = a_trt_so * val;
    grad_val.at(7) = b_trt * val;
    grad_val.at(8) = b_trt_so * val;
    return grad_val;
}

std::vector<double> InfStateSepSoModel::rec_b_grad(
    ...
    grad_val.at(6) = b_trt_so * val;
    return grad_val;
}


std::vector<double> InfStateSepSoModel::inf_b_hess(const uint32_t & b_node,
        const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
    ...
    const std::vector<double> inner_grad({1, 0, 0,
                    0, 0,
                    0, 0,
                    static_cast<double>(b_trt), static_cast<double>(b_trt_so)});
...
std::vector<double> InfStateSepSoModel::a_inf_b_hess(
        const uint32_t & a_node, const uint32_t & b_node,
        const bool & a_trt, const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    ...
    const std::vector<double> inner_grad({0, 1, 0,
                    static_cast<double>(a_trt), static_cast<double>(a_trt_so),
                    0, 0,
                    static_cast<double>(b_trt), static_cast<double>(b_trt_so)});
...
std::vector<double> InfStateSepSoModel::rec_b_hess(
        const uint32_t & b_node, const bool & b_trt,
        const boost::dynamic_bitset<> & inf_bits,
        const boost::dynamic_bitset<> & trt_bits) const {
    bool b_trt_so = false;
    ...
    const std::vector<double> inner_grad({0, 0, 1,
                    0, 0,
                    static_cast<double>(b_trt), static_cast<double>(b_trt_so),
                    0, 0});
