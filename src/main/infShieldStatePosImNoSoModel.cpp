...
InfShieldStatePosImNoSoModel::InfShieldStatePosImNoSoModel(
...
InfShieldStatePosImNoSoModel::InfShieldStatePosImNoSoModel(
        const InfShieldStatePosImNoSoModel & other)
...
InfShieldStatePosImNoSoModel::clone() const {
    return std::shared_ptr<Model<InfShieldState> >(
            new InfShieldStatePosImNoSoModel(*this));
}


std::vector<double> InfShieldStatePosImNoSoModel::par() const {
...
void InfShieldStatePosImNoSoModel::par(const std::vector<double> & par) {
...
double InfShieldStatePosImNoSoModel::inf_b(const uint32_t & b_node,
        const bool & b_trt, const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    const bool b_shield = state.shield.at(b_node) > 0;

    const double base = this->intcp_inf_latent_ +
        this->trt_pre_inf_ * b_trt * (!b_shield);
    LOG_IF(FATAL, !std::isfinite(base)) << "base is not finite.";
    return 1.0 - 1.0 / (1.0 + std::exp(std::min(100.0, base)));
}

double InfShieldStatePosImNoSoModel::a_inf_b(
        const uint32_t & a_node, const uint32_t & b_node,
        const bool & a_trt, const bool & b_trt,
        const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    const bool a_shield = state.shield.at(a_node) > 0;
    const bool b_shield = state.shield.at(b_node) > 0;

    const double base = this->intcp_inf_ +
        this->trt_act_inf_ * a_trt * (!a_shield)
        + this->trt_pre_inf_ * b_trt * (!b_shield);
...
double InfShieldStatePosImNoSoModel::rec_b(const uint32_t & b_node,
        const bool & b_trt, const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    const bool b_shield = state.shield.at(b_node) > 0;

    const double base = this->intcp_rec_ +
        this->trt_act_rec_ * b_trt * (!b_shield);
...
std::vector<double> InfShieldStatePosImNoSoModel::inf_b_grad(
    ...
    grad_val.at(0) = val;
    grad_val.at(5) = b_trt * val * (!b_shield);
    return grad_val;
}

std::vector<double> InfShieldStatePosImNoSoModel::a_inf_b_grad(
        const uint32_t & a_node, const uint32_t & b_node,
        const bool & a_trt, const bool & b_trt,
        const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    ...
    std::vector<double> grad_val(this->par_size_, 0.0);
    grad_val.at(1) = val;
    grad_val.at(3) = a_trt * val * (!a_shield);
    grad_val.at(5) = b_trt * val * (!b_shield);
    return grad_val;
}

std::vector<double> InfShieldStatePosImNoSoModel::rec_b_grad(
        const uint32_t & b_node, const bool & b_trt,
        const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    const bool b_shield = state.shield.at(b_node) > 0;
    ...
    std::vector<double> grad_val(this->par_size_, 0.0);
    grad_val.at(2) = val;
    grad_val.at(4) = b_trt * val * (!b_shield);
    return grad_val;
}


std::vector<double> InfShieldStatePosImNoSoModel::inf_b_hess(
        const uint32_t & b_node, const bool & b_trt,
        const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    const bool b_shield = state.shield.at(b_node) > 0;
    ...
    const std::vector<double> inner_grad({1, 0, 0, 0, 0,
                    static_cast<double>(b_trt * (!b_shield)),
                    0});
...
std::vector<double> InfShieldStatePosImNoSoModel::a_inf_b_hess(
        const uint32_t & a_node, const uint32_t & b_node,
        const bool & a_trt, const bool & b_trt,
        const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    ...
    const std::vector<double> inner_grad({0, 1, 0,
                    static_cast<double>(a_trt * (!a_shield)),
                    0, static_cast<double>(b_trt * (!b_shield)),
                    0});
...
std::vector<double> InfShieldStatePosImNoSoModel::rec_b_hess(
        const uint32_t & b_node, const bool & b_trt,
        const InfShieldState & state,
        const boost::dynamic_bitset<> & trt_bits) const {
    const bool b_shield = state.shield.at(b_node) > 0;
    ...
    const std::vector<double> inner_grad({0, 0, 1, 0,
                    static_cast<double>(b_trt * (!b_shield)), 0,
                    0});
...
double InfShieldStatePosImNoSoModel::shield_draw(
...
double InfShieldStatePosImNoSoModel::shield_prob(
...
std::vector<double> InfShieldStatePosImNoSoModel::shield_grad(
        const uint32_t & loc, const InfShieldState & curr_state,
...
std::vector<double> InfShieldStatePosImNoSoModel::shield_hess(
        const uint32_t & loc, const InfShieldState & curr_state,
        const InfShieldState & next_state) const {
    const double & curr_shield = curr_state.shield.at(loc);
