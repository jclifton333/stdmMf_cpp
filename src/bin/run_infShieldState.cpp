...
run(const std::shared_ptr<const Network> & net,
    ...
    // weighted br min length 2
    std::vector<std::future<double> > br_wtd_len_2_val;
    std::vector<std::future<double> > br_wtd_len_2_time;
    for (uint32_t i = 0; i < num_reps; ++i) {
        ++total_sims;
        std::shared_ptr<std::promise<double> > promise_val(
                new std::promise<double>);
        ...
        br_wtd_len_2_val.push_back(promise_val->get_future());
        br_wtd_len_2_time.push_back(promise_time->get_future());
    ...
    {
        const std::string agent_name = "br_wtd_len_2";
        std::vector<double> val(num_reps);
        std::transform(br_wtd_len_2_val.begin(), br_wtd_len_2_val.end(),
                val.begin(), val.begin(),
                [](std::future<double> & a, const double & b) {
                    return a.get();
                });
        std::vector<double> time(num_reps);
        std::transform(br_wtd_len_2_time.begin(), br_wtd_len_2_time.end(),
                time.begin(), time.begin(),
                [](std::future<double> & a, const double & b) {
                    return a.get();
                });
        const std::pair<double, double> br_wtd_len_2_stats =
            mean_and_var(val);
        const std::vector<double> agent_res =
            {br_wtd_len_2_stats.first,
             std::sqrt(br_wtd_len_2_stats.second / num_reps),
             mean_and_var(time).first};
        all_results.push_back(std::pair<std::string, std::vector<double> >
                (agent_name, agent_res));
    }
